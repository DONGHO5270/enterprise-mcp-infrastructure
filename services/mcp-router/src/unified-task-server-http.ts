...
class UnifiedTaskServerHTTP {
   ...
   */
  private async callMCPService(service: string, method: string, params: any): Promise<any> {
    try {
      const response = await axios.post(
        `${this.baseUrl}/mcp/${service}`,
        {
          jsonrpc: '2.0',
          id: Date.now(),
          method,
  ...
  private async handleInitialize() {
    ...
    const toolsPromises = corServices.map(service =>
      this.getServiceTools(service).catch(() => [])
    );
