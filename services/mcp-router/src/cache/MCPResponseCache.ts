...
interface CachePolicy {
  ...
  maxSize: number;    // Max cache size per service
  enabled: boolean;
}

export class MCPResponseCache extends EventEmitter {
   ...
   * 캐시에서 응답 조회
   */
  async get(service: string, method: string, params: any): Promise<any | null> {
   ...
   * 캐시에 응답 저장
   */
  async set(service: string, method: string, params: any, value: any): Promise<void> {
   ...
   * 특정 서비스의 캐시 무효화
   */
  async invalidate(service?: string): Promise<void> {
    if (!service) {
      // 전체 캐시 무효화
   ...
   * 가장 많이 캐시된 서비스 통계
   */
  private getTopServices(): Array<{ service: string; count: number; size: number }> {
...
export function cacheMiddleware() {
  return async (req: any, res: any, next: any) => {
    ...
    if (method && method.startsWith('tools/list') || method === 'resources/list') {
      const cached = await mcpCache.get(service, method, params);
      if (cached) {
        res.json(cached);
        return;
      }
    ...
    res.json = function(data: any) {
      if (!data.error && method) {
        mcpCache.set(service, method, params, data);
      }
