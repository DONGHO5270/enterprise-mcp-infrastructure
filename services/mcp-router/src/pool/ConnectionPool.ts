...
interface PoolConfig {
  minSize: number;          // 최소 연결 수
  maxSize: number;          // 최대 연결 수
  acquireTimeout: number;   // 연결 획득 타임아웃
  idleTimeout: number;      // 유휴 타임아웃
  healthCheckInterval: number; // 헬스체크 간격
  maxRequestsPerConnection: number; // 연결당 최대 요청 수
}

export class ConnectionPool extends EventEmitter {
  private connections: Map<string, PooledConnection[]> = new Map();
  private waitingQueue: Map<string, Array<(conn: PooledConnection) => void>> = new Map();
  private healthCheckTimers: Map<string, NodeJS.Timeout> = new Map();
  
  // 서비스별 풀 설정
  private static configs: Map<string, PoolConfig> = new Map([
    ['clear-thought', {
      minSize: 2,
      maxSize: 5,
      acquireTimeout: 5000,
   ...
   * 연결 획득
   */
  async acquire(serviceName: string, service: MCPService): Promise<PooledConnection> {
  ...
  private async initializePool(
    ...
    logger.info(`[🏊 Pool] Initialized ${serviceName} with ${config.minSize} connections`);
  }
  ...
  private async createConnection(
    ...
    childProcess.stderr?.on('data', (data) => {
      logger.debug(`[Pool ${id}] stderr: ${data.toString()}`);
    });
    
    // 에러 핸들러
    childProcess.on('error', (error) => {
      logger.error(`[Pool ${id}] Process error:`, error);
    ...
    childProcess.on('exit', (code) => {
      logger.info(`[Pool ${id}] Process exited with code ${code}`);
   ...
   * 연결 초기화
   */
  private async initializeConnection(connection: PooledConnection): Promise<void> {
    const initRequest = {
      jsonrpc: '2.0',
      id: `init-${Date.now()}`,
      method: 'initialize',
   ...
   * 버퍼 처리
   */
  private processBuffer(connection: PooledConnection): void {
    ...
    for (const line of lines) {
      if (line.trim()) {
        try {
          ...
          logger.warn(`[Pool ${connection.id}] JSON parse error:`, error);
        }
      }
    }
  }
   ...
   * 헬스체크 시작
   */
  private startHealthCheck(serviceName: string, config: PoolConfig): void {
      ...
      for (const connection of pool) {
        if (!connection.inUse) {
          try {
            ...
            connection.healthCheckFailures++;
            logger.warn(`[Pool ${connection.id}] Health check failed:`, error);
   ...
   * 헬스체크 실행
   */
  private async healthCheck(connection: PooledConnection): Promise<void> {
    const healthRequest = {
      jsonrpc: '2.0',
      id: `health-${Date.now()}`,
      method: 'tools/list',
   ...
   * 연결 에러 처리
   */
  private handleConnectionError(serviceName: string, connection: PooledConnection): void {
    // 보류 중인 요청 모두 거부
   ...
   * 연결 제거
   */
  private removeConnection(serviceName: string, connection: PooledConnection): void {
    const pool = this.connections.get(serviceName);
    if (!pool) return;
  ...
  private cleanup(): void {
    ...
    for (const [serviceName, pool] of this.connections) {
      ...
      for (const connection of idleConnections) {
        this.removeConnection(serviceName, connection);
        logger.info(`[🧹 Pool] Removed idle connection ${connection.id}`);
      }
    }
  }
  ...
  async shutdown(): Promise<void> {
    ...
    for (const [serviceName, pool] of this.connections) {
      ...
      logger.info(`[Pool] Shutdown ${serviceName}: ${pool.length} connections closed`);
    }
